...
A1(N_nodes,N_nodes)=0;A2(N_nodes,N_nodes)=0;A3(N_nodes,N_nodes)=0;
...
% val=[max(h1.Values);max(h2.Values);max(h3.Values)];%;max(h4.Values);max(h5.Values)];
% mval=max(val);
% line([Jp-J Jp-J],[0 mval],'Color','r','LineWidth',2);
...
% title('NYCM tweets: reply, retweet, mention')
%%
%% 
Delta=Jp-J;
Delta_M=Jps-Js;
...
MM=find_maxmed_rn(AA1,AA2,AA3);
MS=find_maxsyn_rn(AA1,AA2,AA3);
%end
...
axis square
set(gca,'LooseInset',get(gca,'TightInset'));
set(gca,'Color','none');%xlim([-0.01 1]);ylim([-0.01 1]);% legend('\color[rgb]{0,0,0} random','\color[rgb]{.5 .9 .2} mediation','\color[rgb]{1 0 1} suppression','\color[rgb]{0.9020,0.6235,0} C Elegans','\color[rgb]{ 0.8000,0.4745,0.6549} Lazega law firm','\color[rgb]{0,0.6196,0.4510} NYC2014 Twitter');